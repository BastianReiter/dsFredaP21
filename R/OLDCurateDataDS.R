...
#'                                            \item Process \code{data.frame} - Default: \code{dsFredaP21::Set.DataHarmonization}
...
#'                                            \item Dictionary \code{data.frame} - Default: \code{dsFredaP21::Set.Dictionary}
#'                                            \item Dictionary.Profile \code{string} - Profile used in \emph{Dictionary} - Default: 'Default'
#'                                            \item FuzzyStringMatching \code{data.frame} - Default: \code{dsFredaP21::Set.FuzzyStringMatching}
...
#'                                  \itemize{ \item RuleSet \code{data.frame} - Default: \code{dsFredaP21::Set.FeatureObligations}
...
#'                                  \itemize{ \item RuleSet \code{data.frame} - Default: \code{dsFredaP21::Set.FeatureTracking}
...
#-------------------------------------------------------------------------------
  ...
  if (is.null(Settings$DataHarmonization$Process)) { Settings$DataHarmonization$Process <- dsFredaP21::Set.DataHarmonization }
  if (is.null(Settings$DataHarmonization$Process.Profile)) { Settings$DataHarmonization$Process.Profile <- "Default" }
  if (is.null(Settings$DataHarmonization$TransformativeExpressions)) { Settings$DataHarmonization$TransformativeExpressions <- dsFredaP21::Set.TransformativeExpressions }
  ...
  if (is.null(Settings$DataHarmonization$Dictionary)) { Settings$DataHarmonization$Dictionary <- dsFredaP21::Set.Dictionary }
  if (is.null(Settings$DataHarmonization$Dictionary.Profile)) { Settings$DataHarmonization$Dictionary.Profile <- "Default" }
  if (is.null(Settings$DataHarmonization$FuzzyStringMatching)) { Settings$DataHarmonization$FuzzyStringMatching <- dsFredaP21::Set.FuzzyStringMatching }
  ...
  if (is.null(Settings$FeatureObligations$RuleSet)) { Settings$FeatureObligations$RuleSet <- dsFredaP21::Set.FeatureObligations }
  if (is.null(Settings$FeatureObligations$RuleSet.Profile)) { Settings$FeatureObligations$RuleSet.Profile <- "Default" }
  if (is.null(Settings$FeatureTracking$RuleSet)) { Settings$FeatureTracking$RuleSet <- dsFredaP21::Set.FeatureTracking }
...
# If list of 'Settings' passed to function is incomplete, complete it with default values
if (is.null(Settings$DataHarmonization$Run)) { Settings$DataHarmonization$Run <- TRUE }
if (is.null(Settings$DataHarmonization$Methods)) { Settings$DataHarmonization$Methods <- dsFredaP21::Meta_DataHarmonizationMethods }
if (is.null(Settings$DataHarmonization$TransformativeExpressions)) { Settings$DataHarmonization$TransformativeExpressions <- dsFredaP21::Meta_TransformativeExpressions }
if (is.null(Settings$DataHarmonization$TransformativeExpressions.Profile)) { Settings$DataHarmonization$TransformativeExpressions.Profile <- "Default" }
if (is.null(Settings$DataHarmonization$Dictionary)) { Settings$DataHarmonization$Dictionary <- dsFredaP21::Meta_Dictionary }
if (is.null(Settings$DataHarmonization$Dictionary.Profile)) { Settings$DataHarmonization$Dictionary.Profile <- "Default" }
if (is.null(Settings$DataHarmonization$FuzzyStringMatching)) { Settings$DataHarmonization$FuzzyStringMatching <- dsFredaP21::Meta_FuzzyStringMatching }
if (is.null(Settings$DataHarmonization$FuzzyStringMatching.Profile)) { Settings$DataHarmonization$FuzzyStringMatching.Profile <- "Default" }
if (is.null(Settings$FeatureObligations$RuleSet)) { Settings$FeatureObligations$RuleSet <- dsFredaP21::Meta_FeatureObligations }
if (is.null(Settings$FeatureObligations$RuleSet.Profile)) { Settings$FeatureObligations$RuleSet.Profile <- "Default" }
if (is.null(Settings$FeatureTracking$RuleSet)) { Settings$FeatureTracking$RuleSet <- dsFredaP21::Meta_FeatureTracking }
if (is.null(Settings$FeatureTracking$RuleSet.Profile)) { Settings$FeatureTracking$RuleSet.Profile <- "Default" }
if (is.null(Settings$TableCleaning$Run)) { Settings$TableCleaning$Run <- TRUE }
if (is.null(Settings$TableNormalization$Run)) { Settings$TableNormalization$Run <- TRUE }
if (is.null(Settings$TableNormalization$RuleSet)) { Settings$TableNormalization$RuleSet <- dsFredaP21::Meta_TableNormalization }
if (is.null(Settings$TableNormalization$RuleSet.Profile)) { Settings$TableNormalization$RuleSet.Profile <- "Default" }
...
AllTableNames <- dsFredaP21::Meta_Tables$TableName_Curated
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        EligibleFeatureNames <- filter(dsFredaP21::Meta_Features, TableName_Curated == tablename)$FeatureName_Raw
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        vc_Lookup <- filter(dsFredaP21::Meta_Features, TableName_Curated == tablename)$FeatureName_Raw
                        names(vc_Lookup) <- filter(dsFredaP21::Meta_Features, TableName_Curated == tablename)$FeatureName_Curated
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        RequiredFeatureNames <- dplyr::filter(dsFredaP21::Meta_Features, TableName_Curated == tablename)$FeatureName_Curated
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (!(is.null(Table) | length(Table) == 0 | nrow(Table) == 0))
                        {
                            ...
                            # Clean current table using auxiliary function dsFredaP21::CleanTable() (Can be optionally omitted)
                            if (Settings$TableCleaning$Run == TRUE)
                            {
                                Table <- Table %>%
...
if (all(names(DataSet) == names(ls_MonitorMetaData)))
{
    ...
    ls_ValueCounts_Raw <- map2(.x = DataSet,
                               ...
                               .f = function(DataFrame, MonitorMetaData)
                                    {
                                       DataFrame %>%
                                           dsFredaP21::TrackValueCounts(FeatureNames = names(MonitorMetaData),
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (Settings$DataHarmonization$Run == TRUE)      # The check is placed here and not before the mapping function so that the 'TrackID' is appended regardless (see above)
                        {
                            ...
                            EligibleValueSets <- dsFredaP21::Meta_Values %>%
                                                      filter(Table == tablename) %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   - Recoding data using dsFredaP21::RecodeData() based on specifications in dsFredaP21::Meta_Values
#   - RecodeData() uses a dictionary in the form of a named vector to perform recoding on a target vector
#   - Format / Re-type data using dsFredaP21::FormatData() based on specifications in dsFredaP21::Meta_Features
#-------------------------------------------------------------------------------
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (!(nrow(Table) == 0))
                        {
                            ...
                            FeaturesWithValueSets <- dsFredaP21::Meta_Values %>%
                                                          filter(Table == tablename) %>%
                                                          pull(Feature) %>%
                                                          unique()
                            ...
                            {
                                RecodingDictionaries <- dsFredaP21::Meta_Values %>%
                                                            filter(Table == tablename) %>%
                                                            split(.$Feature) %>%      # 'split' is a base function and needs '.$' to address 'Feature'
                            ...
                            FeatureTypes <- dsFredaP21::Meta_Features %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Module D 7)  Finalize transformation of data values using dsFredaP21::FinalizeDataTransformation()
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#   - All predefined information stored in dsFredaP21::Meta_Values
#-------------------------------------------------------------------------------
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (!(nrow(Table) == 0))
                        {
                            ...
                            if (length(FeaturesWithValueSets) > 0)
                            {
                                for (i in 1:length(FeaturesWithValueSets))
                                {
                                    EligibleValueSet <- dsFredaP21::Meta_Values %>%
                                                            filter(Table == tablename,
...
# Using dsFredaP21::FindRedundantEntries(), mark redundant entries in table 'Case' for further processing
Aux_DiagnosisRedundancies <- DataSet$Case %>%
                                  FindRedundantEntries(PrimaryKeyFeature = "CasePseudonym",
                                                       DiscriminatoryFeatures = Meta_Features %>%
                                                                                    filter(TableName_Curated == "Case", IsDiscriminatory == TRUE) %>%
                                                                                    pull(FeatureName_Curated),
                                                       EssentialFeatures = Meta_Features %>%
                                                                                filter(TableName_Curated == "Case", IsEssential == TRUE) %>%
                                                                                pull(FeatureName_Curated),
...
# Find and remove redundant entries in other tables
DataSet <- DataSet %>%
                imap(function(Table, tablename)
                     {
                        if (nrow(Table) > 0 & tablename != "Case")
                        {
                            try(ProgressBar$tick())
