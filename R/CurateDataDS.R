...
#-------------------------------------------------------------------------------
  ...
  AllTableNames <- dsFredaP21::Meta.Tables$TableName.Curated
...
# Rename features from raw feature names to curated feature names
#-------------------------------------------------------------------------------
                  ...
                  imap(function(Table, tablename)
                       {
                          ...
                          vc_Lookup <- filter(dsFredaP21::Meta.Features, TableName.Curated == tablename)$FeatureName.Raw
                          names(vc_Lookup) <- filter(dsFredaP21::Meta.Features, TableName.Curated == tablename)$FeatureName.Curated
                  ...
                  imap(function(Table, tablename)
                       {
                          ...
                          if (Settings$DataHarmonization$Run == TRUE)      # The check is placed here and not before the mapping function so that the 'TrackID' is appended regardless (see above)
                          {
                              ...
                              for (featurename in HarmonizationProcess$Feature)
                              {
                                  ...
                                  EligibleValueSet <- dsFredaP21::Meta.Values %>%
                                                          filter(Table == tablename,
                                                                 FeatureName.Curated == featurename) %>%
                                                          pull(Value.Curated)
...
#===============================================================================
#   - Recoding data using dsFreda::RecodeData() based on specifications in dsFredaP21::Meta.Values
#   - RecodeData() uses a dictionary in the form of a named vector to perform recoding on a target vector
#   - Format / Re-type data using dsFreda::FormatData() based on specifications in dsFredaP21::Meta.Features
#-------------------------------------------------------------------------------
                  ...
                  imap(function(Table, tablename)
                       {
                          ...
                          if (length(Table) > 0 && nrow(Table) > 0)
                          {
                              ...
                              FeaturesWithValueSets <- dsFredaP21::Meta.Values %>%
                                                            filter(Table == tablename) %>%
                              ...
                              {
                                  RecodingDictionaries <- dsFredaP21::Meta.Values %>%
                                                              filter(Table == tablename) %>%
                              ...
                              FeatureTypes <- dsFredaP21::Meta.Features %>%
...
#   - All predefined information stored in dsFredaP21::Meta.Values
#-------------------------------------------------------------------------------
