...
#'                                            \item Methods \code{data.frame} - Default: \code{dsP21Curator::Meta_DataHarmonizationMethods}
#'                                            \item TransformativeExpressions \code{data.frame} - Default: \code{dsP21Curator::Meta_TransformativeExpressions}
#'                                            \item TransformativeExpressions.Profile \code{string} - Profile used in \emph{TransformativeExpressions} - Default: 'Default'
#'                                            \item Dictionary \code{data.frame} - Default: \code{dsP21Curator::Meta_Dictionary}
#'                                            \item Dictionary.Profile \code{string} - Profile used in \emph{Dictionary} - Default: 'Default'
#'                                            \item FuzzyStringMatching \code{data.frame} - Default: \code{dsP21Curator::Meta_FuzzyStringMatching}
...
#'                                  \itemize{ \item RuleSet \code{data.frame} - Default: \code{dsP21Curator::Meta_FeatureObligations}
...
#'                                  \itemize{ \item RuleSet \code{data.frame} - Default: \code{dsP21Curator::Meta_FeatureTracking}
...
#'                                            \item RuleSet \code{data.frame} - Deault: \code{dsP21Curator::Meta_TableNormalization}
...
CurateDataDS <- function(RawDataSetName.S = "RawDataSet",
                         Settings.S = list(DataHarmonization = list(Run = TRUE,
                                                                    Methods = dsP21Curator::Meta_DataHarmonizationMethods,
                                                                    TransformativeExpressions = dsP21Curator::Meta_TransformativeExpressions,
                                                                    TransformativeExpressions.Profile = "Default",
                                                                    Dictionary = dsP21Curator::Meta_Dictionary,
                                                                    Dictionary.Profile = "Default",
                                                                    FuzzyStringMatching = dsP21Curator::Meta_FuzzyStringMatching,
                                                                    FuzzyStringMatching.Profile = "Default"),
                                           FeatureObligations = list(RuleSet = dsP21Curator::Meta_FeatureObligations,
                                                                     RuleSet.Profile = "Default"),
                                           FeatureTracking = list(RuleSet = dsP21Curator::Meta_FeatureTracking,
                                                                  RuleSet.Profile = "Default"),
                                           TableCleaning = list(Run = TRUE),
                                           TableNormalization = list(Run = TRUE,
                                                                     RuleSet = dsP21Curator::Meta_TableNormalization,
                                                                     RuleSet.Profile = "Default")),
...
if (is.null(Settings$DataHarmonization$Methods)) { Settings$DataHarmonization$Methods <- dsP21Curator::Meta_DataHarmonizationMethods }
if (is.null(Settings$DataHarmonization$TransformativeExpressions)) { Settings$DataHarmonization$TransformativeExpressions <- dsP21Curator::Meta_TransformativeExpressions }
...
if (is.null(Settings$DataHarmonization$Dictionary)) { Settings$DataHarmonization$Dictionary <- dsP21Curator::Meta_Dictionary }
if (is.null(Settings$DataHarmonization$Dictionary.Profile)) { Settings$DataHarmonization$Dictionary.Profile <- "Default" }
if (is.null(Settings$DataHarmonization$FuzzyStringMatching)) { Settings$DataHarmonization$FuzzyStringMatching <- dsP21Curator::Meta_FuzzyStringMatching }
...
if (is.null(Settings$FeatureObligations$RuleSet)) { Settings$FeatureObligations$RuleSet <- dsP21Curator::Meta_FeatureObligations }
if (is.null(Settings$FeatureObligations$RuleSet.Profile)) { Settings$FeatureObligations$RuleSet.Profile <- "Default" }
if (is.null(Settings$FeatureTracking$RuleSet)) { Settings$FeatureTracking$RuleSet <- dsP21Curator::Meta_FeatureTracking }
...
if (is.null(Settings$TableNormalization$RuleSet)) { Settings$TableNormalization$RuleSet <- dsP21Curator::Meta_TableNormalization }
...
AllTableNames <- dsP21Curator::Meta_Tables$TableName_Curated
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        vc_Lookup <- filter(dsP21Curator::Meta_Features, TableName_Curated == tablename)$FeatureName_Raw
                        names(vc_Lookup) <- filter(dsP21Curator::Meta_Features, TableName_Curated == tablename)$FeatureName_Curated
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        RequiredFeatureNames <- dplyr::filter(dsP21Curator::Meta_Features, TableName_Curated == tablename)$FeatureName_Curated
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (!(is.null(Table) | length(Table) == 0 | nrow(Table) == 0))
                        {
                            ...
                            # Clean current table using auxiliary function dsP21Curator::CleanTable() (Can be optionally omitted)
                            if (Settings$TableCleaning$Run == TRUE)
                            {
                                Table <- Table %>%
                          ...
                          map(function(tablename)
                              {
                                  ...
                                  ls_EligibleValues <- vc_FeaturesToTrack %>%
                                                            map(function(featurename)
                                                                {
                                                                    Values <- dsP21Curator::Meta_Values %>%
                                                                                  filter(Table == tablename,
                                                                                         Feature == featurename) %>%
                                                                                  select(Value_Raw,
                                                                                         Value_Curated)
...
if (all(names(DataSet) == names(ls_MonitorMetaData)))
{
    ...
    ls_ValueCounts_Raw <- map2(.x = DataSet,
                               ...
                               .f = function(DataFrame, MonitorMetaData)
                                    {
                                       DataFrame %>%
                                           dsP21Curator::TrackValueCounts(FeatureNames = names(MonitorMetaData),
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (Settings$DataHarmonization$Run == TRUE)      # The check is placed here and not before the mapping function so that the 'TrackID' is appended regardless (see above)
                        {
                            ...
                            EligibleValueSets <- dsP21Curator::Meta_Values %>%
                                                      filter(Table == tablename) %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   - Recoding data using dsP21Curator::RecodeData() based on specifications in dsP21Curator::Meta_Values
#   - RecodeData() uses a dictionary in the form of a named vector to perform recoding on a target vector
#   - Format / Re-type data using dsP21Curator::FormatData() based on specifications in dsP21Curator::Meta_Features
#-------------------------------------------------------------------------------
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (!(nrow(Table) == 0))
                        {
                            ...
                            FeaturesWithValueSets <- dsP21Curator::Meta_Values %>%
                                                          filter(Table == tablename) %>%
                                                          pull(Feature) %>%
                                                          unique()
                            ...
                            {
                                RecodingDictionaries <- dsP21Curator::Meta_Values %>%
                                                            filter(Table == tablename) %>%
                                                            split(.$Feature) %>%      # 'split' is a base function and needs '.$' to address 'Feature'
                            ...
                            FeatureTypes <- dsP21Curator::Meta_Features %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Module D 7)  Finalize transformation of data values using dsP21Curator::FinalizeDataTransformation()
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#   - All predefined information stored in dsP21Curator::Meta_Values
#-------------------------------------------------------------------------------
                ...
                imap(function(Table, tablename)
                     {
                        ...
                        if (!(nrow(Table) == 0))
                        {
                            ...
                            if (length(FeaturesWithValueSets) > 0)
                            {
                                for (i in 1:length(FeaturesWithValueSets))
                                {
                                    EligibleValueSet <- dsP21Curator::Meta_Values %>%
                                                            filter(Table == tablename,
...
# Using dsP21Curator::FindRedundantEntries(), mark redundant entries in table 'Case' for further processing
